...
- ☐ **W2 — плавающие фигуры/текстбоксы на листе.** `xdr:sp` (spPr/geom/fill/line + txBody) → `ShapeBlock`,
  переиспользуя DrawingML-машину docx/pptx.
- ☐ **W3 — ссылки на ячейках.** worksheet `<hyperlinks r:id|location>` → `Run.href` (PDF /Link + HTML `<a>`).
- ☐ **W4 — колонтитулы листа.** `<headerFooter><oddHeader/oddFooter>` коды `&L/&C/&R`, `&P/&N/&D/&F` → HF-
  band'ы (машина уже есть для docx).
- ☐ **W5 — доп. типы условного форматирования.** `top10`, `aboveAverage`, `duplicate/uniqueValues`, text-
  based (`containsText`/`beginsWith`/`endsWith`), `timePeriod`, `expression` (формула).
- ☐ **W6 — детали формата ячеек.** in-cell rich text (per-run bold/цвет), `textRotation`, диагональные
  границы, не-solid/градиентные заливки, `indent`/`shrinkToFit`/wrapText.
- ☐ **W7 — комментарии/заметки к ячейкам.** legacy (`xl/comments` + VML) + threaded (`xl/threadedComments`).
- ☐ **W8 — form/ActiveX контролы.** чекбоксы/радио/спиннеры (низкий приоритет, может остаться).
