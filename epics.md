...
- **PV4 (частично) ✓ — доки.** `scope.md` синхронизирован под реальность (кросс-катящий, для
  ...
  E-COMMENTS CM3 «scope.md», и стаределость E-SMARTART. Доки-онли (кода нет). Осталось PV4:
  эмфаза гранд-итоговой КОЛОНКИ, outline-отступы, демо. Напоминание: на сайт попадёт только
  после мёрджа v1→main + редеплоя (действие пользователя).
