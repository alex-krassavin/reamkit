...
- **XLS-8 ✓ — гиперссылки `.xls` (HLINK, закрытие Not-yet).** biff-reader парсит HLINK-рекорд (0x01B8): Ref8U-
  диапазон + StdHlink CLSID + Hyperlink Object ([MS-OSHARED] §2.3.7.1) — streamVersion 2, hlinkFlags, опц.
  display/frame-строки (skip), затем moniker. Для внешнего URL: oleMoniker с URLMoniker CLSID → NUL-terminated
  UTF-16LE URL (§2.3.7.6). `parseHlink` → `SheetHyperlink{ref:MergedRange,url}` прямо в IR (URL inline, без rels
  как в xlsx), который проекция уже потребляет (`hyperlinkUrlAt` → `run.href` на покрытых ячейках → PDF/HTML).
  In-workbook location-only и file/UNC-монишеры пропускаются (missing, не wrong). Researched [MS-XLS]/[MS-OSHARED]
  vs POI HyperlinkRecord. build-xls += `hlinkRec`. +4 теста (13 в xls-reader), 1112 всего. Остаётся `.xls`-
  вторичка: CF/комменты/валидация/defined-names/print-setup.
- **XLS-9 ✓ — page-setup print model `.xls` (закрытие Not-yet).** `parsePrintModel` сканирует sheet-substream и
  заполняет те же grid-поля, что и xlsx-путь, — рендер печати/пагинации работает для legacy-книги идентично:
  SETUP (0xA1: paperSize/iScale/iFitW/iFitH + grbit fPortrait 0x02/fNoPls 0x04/fNoOrient 0x40 → orientation,
  numHdr/numFtr @16/@24 → header/footer-маржины), WSBOOL (0x81, НЕ 0x85=BoundSheet8: fFitToPage 0x0100),
  PRINTGRIDLINES (0x2B), HCENTER/VCENTER (0x83/0x84), LEFT/RIGHT/TOP/BOTTOMMARGIN (0x26-0x29, f64 дюймы),
  HEADER/FOOTER (0x14/0x15, XLUnicodeString; пустой рекорд → нет), H/VPAGEBREAK (0x1B/0x1A → rowBreaks/colBreaks,
  индекс = строка/колонка начала след. страницы = xlsx `<brk id>`). Print_Area/Print_Titles (built-in NAME, нужен
  Rgce-парсер) отложены в defined-names-волну. Researched [MS-XLS] vs POI (исправлен WSBOOL sid: агент дал 0x85,
  но =129=0x81). build-xls += setupRec/wsBoolRec/гридлайны/центрирование/маржины/хедер-футер/брейки. +7 тестов
  (20 в xls-reader), 1119 всего. Остаётся `.xls`-вторичка: CF/комменты/валидация/defined-names.
- **XLS-10 ✓ — defined names + print area/titles `.xls` (закрытие Not-yet).** `buildDefinedNames` парсит
  NAME/Lbl-рекорды (0x18) глобалов → `SheetDoc.definedNames` той же формы, что и xlsx, → проекция резолвит
  Print_Area/Print_Titles тем же `resolvePrintArea`/`resolvePrintTitleRows`. Lbl: grbit(fBuiltin 0x20)/cch@3/
  cce@4/itab@8/Name(XLUnicodeStringNoCch)@14/rgce; built-in id 0x06→`_xlnm.Print_Area`, 0x07→`_xlnm.Print_Titles`
  (иначе skip), regular → имя-строка. `extractNameAreas` — мини ptg-walker: PtgArea3d 0x3B / PtgArea 0x05 /
  PtgRef(3d) / tMemFunc / tUnion; ЛЮБОЙ незнакомый ptg → bail (missing, не wrong). `formatArea`→A1: row-band
  (c1≥255)→"r0:r1", col-band (r1≥65535)→"cA:cB", иначе "A1:D10". itab(1-based, все sheet-типы) → localSheetId
  (0-based, только worksheets) через wsIndex-карту. build-xls += `nameRec` + globalRecords-опция. +4 теста (24 в
  xls-reader), 1123 всего. **`.xls`-print-model завершён** (page-setup XLS-9 + area/titles XLS-10). Остаётся
  `.xls`-вторичка: CF/комменты/валидация.
- **XLS-11 ✓ — комментарии ячеек `.xls` (Note/Obj/Txo, закрытие Not-yet).** `parseComments` собирает Note (0x1C:
  rw@0/col@2/idObj@6/cchAuthor@8/fHighByte@10/author@11, grbit fShow 0x2) + текст из Txo той же Obj, чей FtCmo
  (ft 0x15, ot 0x19=comment, id@6) совпадает с idObj — джойн по shared object id (ключ POI HSSFComment), НЕ по
  порядку записей, так что non-comment Obj в drawing-блоке не сбивает текст. Txo-текст из следующего CONTINUE
  (`[flags][chars]`, как gatherTxoTexts). → `Sheet.comments{ref:A1,author?,text,threaded:false}` → проекция
  рендерит секцию «Comments» после грида (тот же путь, что и xlsx-W7). Note без текста → skip (missing, не
  wrong). Researched [MS-XLS]/POI. build-xls += `noteRec`/`commentObjRec` (+reuse `txoRecs`). +4 теста (28 в
  xls-reader), 1127 всего. Остаётся `.xls`-вторичка: CF/валидация.
- **XLS-12 ✓ — data validation `.xls` (DV, закрытие Not-yet).** `parseDataValidations` собирает DV-рекорды (0x1BE)
  → `grid.dataValidations` той же формы, что и xlsx, → `list`-правило красит in-cell dropdown (тот же
  `listDropdownRanges` в print-model). DV: dwDvFlags u32 (valType биты 0-3 → type; typOperator 20-23 → operator;
  fAllowBlank 0x100; fSuppressCombo 0x200 = ECMA-инвертированный showDropDown; showInput 0x40000; showError
  0x80000), 4× XLUnicodeString (`readXlStringEnd` с end-offset), 2× DVParsedFormula (cce+unused+rgce), SqRefU
  (count + Ref8U). `list` literal из rgce1 PtgStr (0x17 + ShortXLUnicodeString) → `"a,b,c"`, либо area→A1 через
  `extractNameAreas`/`formatArea`. showDropDown=fSuppressCombo выровнен с ECMA-инверсией (1 ПРЯЧЕТ). Researched
  [MS-XLS]/POI (исправлен typOperator: 4 бита 0xF00000, не 3). build-xls += `dvRec`. +4 теста (32 в xls-reader),
  1131 всего. **`.xls`-вторичка почти закрыта** — остаётся только CF.
- **XLS-13 ✓ — conditional formatting `.xls` (CondFmt+CF, закрытие `.xls`-Not-yet).** `parseConditionalFormats`:
  CondFmt (0x1B0: ccf + Ref8U-bbox + SqRefU-диапазоны) + следующие ccf CF-рекордов (0x1B1) → `grid.conditional
  Formats` той же формы, что и xlsx. CF: ct(1: cellIs / 2: expression), cp→CfOperator, cce1/cce2, DXFN, rgce1/2.
  DXFN walk: option-flags dword + fixed-size sub-blocks (font 118 / align 8 / bord 8 / pat 4 / prot 2, POI-маски
  0x04000000…); цвет читается ТОЛЬКО если walk попадает ровно в rgce1Start (иначе unmodelled-блок → без цвета,
  graceful). Заливка: dxfpat u32 (fls<<10 | icvFg<<16 | icvBg<<23) → `Dxf.fill{patternType:'solid',fgColorHex}`,
  fg = видимый цвет solid-заливки; шрифт-цвет icvFore@80 (i32). Формулы-операнды якорятся ОТ ХВОСТА (recLen−cce2−
  cce1), так что неверный размер DXFN не портит rgce; PtgInt/PtgNum/PtgStr → текст-константа. dxfId индексирует
  общий workbook-`dxfs` (мерджится в styles), палитра — экспортированный `buildBiffPalette` (рефактор biff-styles).
  CF12 (colorScale/dataBar/iconSet, 0x087A/0x087B) НЕ читается — он есть только в `.xls` от Excel 2007+ (в 97–2003
  UI таких правил не было); записи скипаются gracefully. Researched [MS-XLS]/POI (CFRuleRecord/DXFN/DXFPat).
  build-xls += `condFmtRec`/`cfRec`. +4 теста (36 в xls-reader), 1135 всего. **`.xls`-Not-yet закрыт полностью**
  (кроме CF12-ниши). `.ppt`-custom-geometry + `.doc` (list-format, table-borders) — остаток Not-yet.
...
- **DOC-8 ✓ — колонтитулы `.doc` (PlcfHdd).** Текст колонтитулов — отдельные «истории» в том же piece-table-
  стриме, ПОСЛЕ основного текста и сносок (CP ≥ ccpText+ccpFtn). `buildParagraphs` рефакторнут на CP-окно
  (cpStart, cpEnd) с фаст-форвардом по pieces. `extractHeaderFooters`: FIB ccpFtn/ccpHdd @0x50/0x54 +
  fcPlcfHdd/lcb @0xF2/0xF6 → PlcfHdd (плекс CP, без данных) → истории 6–11 секции 0 (even/odd/first × hdr/ftr);
  каждую парсим как окно [hddStart+cp[i], hddStart+cp[i+1]); пустые (только CR) дропаются. doc-reader строит
  `FlowDoc.headersFooters` (id `header-default`/…) + `section.headers/footers` рефы (odd→default, +first/even).
  **Best-effort:** порядок историй PlcfHdd не проверить на реальном файле → грейсфул при любой структурной
  аномалии. Фикстура: opts.headerFooter → header-стрим как доп-piece + PlcfHdd + ccpText=mainChars/ccpHdd.
  `tests/doc-reader.test.ts` +3 (default hdr+ftr / first-page / нет колонтитулов), 29 всего. Списки/бордюры-
  ячеек — последние волны.
- **DOC-9 ✓ — списки `.doc` (последняя волна эпика).** decodePapxGrpprl читает sprmPIlvl (0x260A → уровень) +
  sprmPIlfo (0x460B u16 → list-override; >0 ⇒ list item) → `DocParaProps.listIlfo/listIlvl`. doc-reader
  `mapParagraph`: для list-абзаца префиксит ран-маркер (буллет по уровню `•`/`◦`/`▪`) + indentLeft =
  (ilvl+1)·18pt. **Bounded/best-effort:** точный numbered-формат (таблицы LST/LVL/LFO + неуверенные FIB-офсеты
  fcPlcfLst/fcPlfLfo) НЕ парсится — нумерованные списки рендерятся буллетами (как бордюры/merge ячеек —
  задокументированный gap); грейсфул (нет ilfo → обычный абзац). Фикстура: DocParaFormat += listIlfo/listIlvl →
  sprmPIlvl/PIlfo. `tests/doc-reader.test.ts` +1 (2 уровня: `• First`@18pt / `◦ Second`@36pt), 30 всего.
  **`.doc`-эпик завершён** (DOC-1..9: текст + ран/абзац-форматирование + таблицы+ширины + картинки + поля +
  колонтитулы + списки → PDF/SVG/HTML + rewrite в `.docx`). Остаются точный numbered-формат и бордюры/merge ячеек.
- **DOC-10 ✓ — numbered-формат списков `.doc` (закрытие Not-yet).** doc-text `parseListTables` читает list-
  таблицы из Table-стрима через FIB-офсеты fcPlfLst@0x2E2/fcPlfLfo@0x2EA: PlfLfo (lfoMac + 16-байт LFO, lsid@0)
  + PlfLst (cLst i16 + 28-байт LSTF (lsid@0, fSimpleList @26 b0), затем LVL-массив ПОСЛЕ lcbPlfLst). `readLvl`:
  LVLF (iStartAt i32@0, nfc u8@4, cbChpx@24, cbPapx@25) + grpprlPapx + grpprlChpx + Xst (u16 cch + код-юниты;
  ≤8 = плейсхолдер уровня). `DocListTables{lfoLsids, lists:Map<lsid,{simple,levels}>}` в DocContent. doc-reader
  `createListMarkers`: per-(lsid,level) счётчики; маркер = подстановка плейсхолдеров (число уровня, формат по nfc
  того уровня) + литералы; nfc 0 arabic / 1-2 roman / 3-4 letter / 22 arabicLZ / 23 bullet (xst-глиф литералом).
  Инкремент уровня + сброс глубже. Researched [MS-DOC] §2.9.{127-131,150} vs POI (исправлены LVLF-офсеты,
  TCGRF/Ico для DOC-11). Bounds-checked → неразрешённый список падает на обычный буллет (missing, не wrong).
  build-doc += `lists`-опция (PlfLfo/PlfLst/LVL). +4 теста (34 в doc), 1139 всего. Остаётся `.doc`: бордюры/
  merge ячеек таблиц.
- **DOC-11 ✓ — бордюры + вертикальный merge ячеек таблиц `.doc` (закрытие Not-yet).** doc-text парсит TC80-массив
  из операнда sprmTDefTable (0xD608): после `itcMac` u8 + (itcMac+1)×i16 границ идут itcMac×20-байт TC80.
  `parseTc80`: tcgrf u16@0 (vertMerge = (tcgrf>>5)&3: 1=continue, 3=restart) + 4×Brc80 (top@4/left@8/bottom@12/
  right@16); `parseBrc80`: dptLineWidth u8@0 (1/8pt), brcType u8@1 (0/0xFF=nil, 3=double), ico u8@2 → `ICO_RGB`
  (17 записей, DkRed@0x0D по errata). `DocTc{borders?,vMerge?}` на `DocParaProps.cellDescriptors` (на TTP-абзаце
  строки). doc-reader переписан на RawCell-конвейер: buildRawRow читает TTP-дескрипторы → `resolveVerticalMerges`
  (проход по столбцу: restart+continue → start/middle/end CellMerge) → toTableCell/toCellBorders (Brc80 → Border
  {style,width pt,colorHex}). Researched [MS-DOC] §2.6.3 (Brc80) / §2.9.{321,345} (TC80/TCGRF) vs POI HWPF.
  Bounds-checked → структурное сомнение даёт missing (не wrong). build-doc += `cellTc`-опция (TC80 после границ).
  +2 теста (36 в doc): per-cell top-border (2pt красный) + vertical merge start/end. **`.doc`-эпик полностью
  завершён** (DOC-1..11). Остаётся только фоновая заливка ячеек (cell shading).
- **PPT-1 ✓ — текст legacy `.ppt` (PowerPoint 97–2003).** Третий бинарный формат на том же CFB-кейстоуне (после
  `.xls`/`.doc`). Новые `src/pptx/ppt/{ppt-text,ppt-reader}.ts`; `pptReader` в DEFAULT_READERS + `ppt:` в
  SOURCE_MIME. `PowerPoint Document`-стрим — дерево рекордов (формат хедера как у Escher: recVer 0xF ⇒ контейнер).
  Слайды достаются косвенно: `Current User` → offsetToCurrentEdit → UserEditAtom-цепочка (offsetLastEdit) →
  PersistDirectoryAtom (persistId low-20 / cnt high-12 → абсолютные офсеты) → DocumentContainer → SlideListWithText
  (instance 0) → SlidePersistAtom.persistIdRef → SlideContainer. Текст: TextCharsAtom (UTF-16) / TextBytesAtom
  (cp1252), split по 0x0D; fallback на outline-текст SLWT и на скан RT_Slide. slideSize (master units = 1/576") →
  pt=units/8 (10×7.5"=5760×4320→720×540). Одна страница на слайд (как pptxReader). Фикстура `build-ppt.ts` через
  `buildCfb`. Researched [MS-PPT] vs POI HSLF. `tests/ppt-reader.test.ts` (15). Encrypted (headerToken 0xF3D1C4DF)
  → dropped-loss. Best-effort/graceful.
- **PPT-2 ✓ — ран/абзац-форматирование `.ppt` (StyleTextPropAtom).** Каждый текст-атом спарен со следующим
  StyleTextPropAtom (0x0FA1): абзац-секция (count u32 + indentLevel u16 + PFMasks u32 + поля) затем символ-секция
  (count u32 + CFMasks u32 + поля), суммы = textLen+1 (фантомный терминатор). Поля читаются в байт-порядке спеки
  (TextPFException/TextCFException — НЕ по возрастанию битов) — каждое present-поле скипается точно. Символ:
  bold/italic/underline (CFStyle, гейт битами 0x3EB7), fontSize (0x20000, points), color (ColorIndexStruct
  index 0xFE → RGB). Абзац: alignment (бит 11, 0/1/2/3/4) + indentLevel. ppt-text отдаёт абзацы из ранов (текст
  режется по символ-ранам); reader → RunProperties/ParagraphProperties. +7 тестов.
- **PPT-3 ✓ — картинки `.ppt` (Pictures-стрим + блипы).** Шейп-блип резолвится: DrawingGroup →
  OfficeArtBStoreContainer → FBSE[i].foDelay (data-офсет 28) = офсет блипа в `Pictures`-стриме; шейп-OPT pib
  (0x4104, low-14 0x0104, 1-based) индексирует store. OfficeArtBlip по foDelay — magic-скан PNG/JPEG за UID-хедером
  (как escher.ts в `.xls`). Картинки слайда — in-flow ImageBlock после текста, размер из intrinsic PNG IHDR /
  JPEG SOF (не prepareImage — он падает на 1×1); page-break держится на абзаце (layout чтит его только там) →
  image-only-слайд получает breaking-anchor. +5 тестов. **`.ppt` фаза-1** (текст+форматирование+картинки).
- **PPT-4 ✓ — placement шейпов `.ppt` (OfficeArtClientAnchor).** Per-slide-модель → `PptShape{rectPt?,
  paragraphs?, image?}`. Обход SpContainer'ов слайда: ClientAnchor (0xF010 — top/left/right/bottom, i16 в
  8-байтной форме / i32 в 16-байтной, master units → pt=units/8) + ClientTextbox (0xF00D) текст + pib картинка.
  Шейп с анкором позиционируется: текст → floating borderless ShapeBlock (абзацы внутри), картинка → floating
  ImageBlock — page-relative по rect (как pptxReader). Без анкора (плейсхолдер наследует master-геометрию, или
  whole-slide fallback) → reading-order flow, поэтому вывод PPT-1..3 не изменился. Page-break на абзаце; floating-
  only-слайд получает breaking-anchor. Researched [MS-PPT]/[MS-ODRAW] vs POI HSLF (порядок полей анкора/единицы) +
  FlowDoc floating-модель (ShapeBlock/FloatAnchor/floatAt). build-ppt += positioned-shape (boxes:
  ClientAnchor+ClientTextbox+pib). +5 тестов, 32 всего в ppt.
- **PPT-5 ✓ — автошейпы `.ppt` (последняя волна эпика).** Анкорный шейп без текста/картинки → декоративный:
  preset из FSP (0xF00A) recInstance (skip group/patriarch/background по FSP-флагам 0x1/0x4/0x400) + литеральные
  fill (OPT 0x0181) / line (OPT 0x01C0) цвета (reuse `optColor` из escher.ts — только sRGB, flags=0; scheme/theme
  → graceful loss). Reader мапит MSOSPT→DrawingML-preset (rect/roundRect/ellipse/diamond/triangle/…; коннекторы
  13/20-49 → line) → positioned vector ShapeBlock (geometry+fill+stroke) по rect. Шейп без литерального fill/line
  пропускается (невидимый). build-ppt += autoshape-эмиттер (FSP + OPT fill/line). +5 тестов, 37 всего в ppt.
  **`.ppt`-эпик (PPT-1..5) завершён** (текст + ран/абзац-форматирование + картинки + placement + автошейпы →
  PDF/SVG/HTML + rewrite в `.docx`). Все три legacy-формата (`.doc`/`.xls`/`.ppt`) читаются на общем CFB-кейстоуне.
- **PPT-6 ✓ — scheme/theme-цвета шейпов `.ppt` (закрытие Not-yet).** `optColor` теперь резолвит scheme-relative
  OfficeArtCOLORREF (флаг fSchemeIndex 0x08 → красный байт = индекс слота 0–7) через цветовую схему слайда:
  SlideSchemeColorSchemeAtom (RT_ColorSchemeAtom 0x07F0, recInstance 1) — собственная схема слайда, либо мастера,
  если slideAtom.slideFlags.fMasterScheme (0x0002) → резолв masterIdRef через persist-директорию до MainMaster.
  8 ColorStruct (r,g,b,unused) → 8 hex; слот 4 = fills. Резолв происходит в ppt-text (модель уже несёт hex), так
  что reader/рендер не меняются. Палитра/системные и текстовые scheme-цвета пока дропаются (missing, не wrong).
  Researched [MS-ODRAW]/[MS-PPT] vs POI (EscherColorRef/ColorSchemeAtom/HSLFSlide). build-ppt += colorScheme/
  followMasterScheme/masters + fill/lineSchemeIndex. +5 тестов, 42 всего в ppt. Остаётся: точная custom-геометрия
  (non-preset → прямоугольник) + палитра/системные цвета.
- **PPT-7 ✓ — точная custom-геометрия freeform-шейпов `.ppt` (закрытие Not-yet).** ppt-text читает комплексные
  OFOPT-свойства pVertices (0x0145) + pSegmentInfo (0x0146): `optComplex` идёт за фиксированными 6-байтными
  записями (комплексные блобы следуют в порядке записей, длина = 4-байтное значение записи). `arrayHeader`
  парсит IMsoArray-заголовок (nElems u16, nElemsAlloc u16, cbElem i16; cbElem<0, напр. 0xFFF0=−16 → (−cbElem)>>2
  байта на элемент — как POI EscherArrayProperty). `parseFreeformGeometry` идёт по опкодам сегментов (старшие
  3 бита = MSOPATHTYPE: lineTo 0, curveTo 1, moveTo 2, close 3, end 4, escape 5), вытягивая точки из pVertices
  (i16-пара @4 байта / i32-пара @8): move/line → 1 точка, curveTo → 3 (кубический безье), close/end → 0. Bounds —
  geoLeft/Top/Right/Bottom (0x0140–0x0143), иначе из min/max вершин; команды в path-space (0..pathWidth) — та же
  IR `CustomGeometry`, что и у pptx custGeom. Escape-сегмент с дугой/эллипсом (sub-code <0x0A) → bail на пресет
  (missing, не wrong); hint-escape (auto/smooth/corner, ≥0x0A) пропускается без расхода вершин. Researched
  [MS-ODRAW] §2.2.{31,32} + POI HSLFAutoShape.getGeometry (PATH_INFO 0xE000 / ESCAPE_INFO 0x1F00) — не
  гадание. ppt-reader: `geometry: { kind:'custom', custom }` вместо пресета у freeform. build-ppt += `freeform`-
  опция (геобокс + pVertices/pSegmentInfo как комплексные OPT-блобы). +5 тестов (треугольник, кубик, hint-escape,
  arc-bail на пресет, PDF-smoke), 47 всего в ppt. **`.ppt`-эпик завершён** (PPT-1..7). Остаётся только палитра/
  системные цвета.
