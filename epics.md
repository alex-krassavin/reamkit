...
**Статус: D1–D7 + T1–T2 ✅ (текстовый writer покрывает реальный документ; гейт зелёный).**
Roundtrip-гейт: 1100 корпус-доков, **ноль writer-падений**, POI 110/110, LO 976/990 полная
...
PDF) и переносит байты как есть → POI ⚠️7→0. T2 — ридерный хвост: docx-reader парсит легаси
VML-картинки (`<w:pict>/<w:object>` → `<v:shape style><v:imagedata r:id>`), размеры из CSS-style;
45 корпус-доков теперь рендерят ранее невидимые VML/ActiveX/OLE-превью. Dangling-rel (байты части
вырезаны) гейтятся — фантом не материализуется. Остаточные v1-пробелы (честные): сноски/чарты/
шейпы/math → лоссы, краевые ±1 параграф (SDT/databinding, гиперссылка на не-тексте, inline/block).
