...
- **SE2 — фрозен-панели в IR.** Парсить `<sheetView><pane>` → `ParsedWorksheet.pane{frozenRows,frozenCols}`;
  писать обратно в xlsx-writer (roundtrip-identity). Без эффекта на PDF (в Excel заморозка не печатается —
  это Print_Titles).
- **SE3 — HTML sticky-панели.** В `convert('html')` рендерить заморозку как `position:sticky` шапку/колонку
  (где заморозка осмысленна — интерактивный вывод). PDF/SVG без изменений.
