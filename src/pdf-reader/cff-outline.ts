...
/**
 * TN 5176 §13 — the NAME a CFF gives each glyph, inverted.
 *
 * A simple font selects its glyphs by name, not by index: `/Differences` says
 * code 2 is `g18`, and only the program's charset says which glyph `g18` is.
 * TAMReview.pdf sets most of its body in a Cambria subset named that way.
 *
 * @param program The raw CFF bytes.
 * @returns Glyph name → glyph index, or `undefined` for a CID-keyed program
 *          (which names nothing) or one that cannot be read.
 */
export function cffNameToGid(program: Uint8Array): Map<string, number> | undefined {
  try {
    const font = parseCff(program);
    return font?.nameToGid;
  } catch {
    return undefined;
  }
}
...
function parseCff(program: Uint8Array): CffFont | undefined {
  ...
  // that SID IS the CID; in any other it is a string, and the string is the
  // glyph's name. Offsets 0..2 name the predefined charsets, which are the
  // standard strings in order.
  const charsetAt = top.get(15)?.[0];
  const sids =
    charsetAt !== undefined && charsetAt > 2
      ? readCharset(program, charsetAt, charStrings.items.length)
      : undefined;
  const cidToGid = isCid ? sids : undefined;
  const nameToGid = isCid ? undefined : namesOf(sids, strings.items, charStrings.items.length);
...
/**
 * TN 5176 §13 — glyph name → glyph index, from the charset's SIDs.
 *
 * An SID below 391 is one of the standard strings; above it indexes the
 * program's own String INDEX. A font with no charset uses ISOAdobe, which is
 * the standard strings in order — so the SID is the glyph index itself.
 */
function namesOf(
  sids: Map<number, number> | undefined,
  strings: ReadonlyArray<Uint8Array>,
  glyphs: number,
): Map<string, number> | undefined {
  const nameOf = (sid: number): string | undefined => {
    if (sid < STANDARD_STRINGS.length) return STANDARD_STRINGS[sid];
    const own = strings[sid - STANDARD_STRINGS.length];
    return own ? new TextDecoder('latin1').decode(own) : undefined;
  };
  const out = new Map<string, number>();
  if (sids) {
    // `readCharset` keys by the value it read and holds the glyph it belongs
    // to, which for a name-keyed font is SID → glyph.
    for (const [sid, gid] of sids) {
      const name = nameOf(sid);
      if (name !== undefined && !out.has(name)) out.set(name, gid);
    }
  } else {
    for (let gid = 0; gid < glyphs && gid < STANDARD_STRINGS.length; gid++) {
      const name = STANDARD_STRINGS[gid];
      if (name !== undefined && !out.has(name)) out.set(name, gid);
    }
  }
...
/**
 * TN 5176 Appendix A — the 391 strings every CFF program shares. An SID below
 * this many is one of these; above it indexes the program's own String INDEX.
 */
const STANDARD_STRINGS: ReadonlyArray<string> = [
  '.notdef',
  'space',
  'exclam',
  'quotedbl',
