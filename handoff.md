...
## v1 — слой IR («Virtual DOM для документов»), ветка `v1`

> **СТАТУС: этапы 1–6 миграции РЕАЛИЗОВАНЫ** (см. ir-design.md §10 — все ✅,
> каждый под байт-в-байт корпус-гейтом; 434 теста). Остаток — стабилизация
> @experimental-схемы (Pt-координаты PageItem, top-left origin, вынос layout в
> src/layout/, resolved-стили в reader'ах) при заморозке контракта.

**ООП/GRASP-волна 1 (выполнена).** По верифицированному анализу oop-design.md: байт-гейт
закреплён в репо (tests/byte-gate.test.ts — sha256-снапшоты PDF, преемственны IR-baseline);
emit-фаза вынесена в src/pdf/styled-page-emitter.ts (вход герметизирован до LaidOutDocument+EmitOptions);
prepareImage/addImage вместо двойного декода с throwaway-PdfDocument (svg-writer берёт mime из prepared);
разорван цикл xlsx-reader↔xlsx-to-pdf (print-model в src/excel/print-model.ts, конвертер 802→102 строки,
мёртвый код удалён); ParseContext вместо протаскивания пары резолверов через 10 сигнатур (квирк parseRun
...
**Корпус-контроль после stage 6 (полный POI, sandbox).** docx: **✅ 67 · ⚠️ 43 · ❌ 17** (бейзлайн 66/44/17:
+1 в ✅, ноль регрессий; все ❌ — те же намеренные safe-reject'ы). xlsx: первый прогон дал **+1 ❌** —
корпус вскрыл реальную регрессию S6-3: `bug62181.xlsx` (874 КБ, ~0.4M ячеек) стал OOM'ить под капом
512 МБ (до S6-3 конвертировался). Три сложившихся пер-ячеечных аллокации, все исправлены (байты PDF
cmp-идентичны до-S6-3-ревизии, гейт 12/12): (1) resolveBodyStyles пересобирал всё дерево → теперь
резолвит IN PLACE (reader владеет свежераспарсенным деревом, перезаписываются только поля properties);
(2) xlsx grid-mapper строил свежие run/paragraph-props на каждую ячейку, убивая identity-мемо каскада →
ячейки одного cellXf разделяют ОДНИ объекты пропсов (резолв-популяция схлопывается до «по одному на xf»);
(3) идемпотентный пере-резолв рендерера над EMPTY-шитом аллоцировал равную копию на пару → reader'ы
регистрируют фикспойнты resolved→resolved в кэшах каскада (пере-резолв = identity-hit);
resolveParagraphProperties получил тот же identity-мемо, что и run-сторона. Финальный xlsx:
**✅ 164 · ⚠️ 164 · ❌ 21** (бейзлайн 163/165/21: +1 в ✅, ❌-состав идентичен — фаззер/бомбы/битые архивы);
bug62181 теперь ⚠️ с TextSim 84% (662k символов; расходится страницами на гиганте — ожидаемо).

**HTML-writer — четвёртый адаптер (FlowDoc→HTML).** Первый writer по FLOW-дереву (svg потребляет PageDoc):
поточная среда — без пагинации, без layout-движка и без шрифтов; `Ream.parse(bytes).convert('html')` —
ноль I/O. Контракт stage-6 окупился напрямую: body уже несёт финальные пропсы и материализованные маркеры
списков — writer мапит значения прямо в CSS; каждое чтение пропсов всё равно идёт через резолвер каскада
над EMPTY-шитом (на reader-выходе это мемоизированный identity-hit через фикспойнт, на сыром дереве —
настоящий резолв, как в PDF-layout). Покрыто: заголовки из outline-level (h1–h6, как в tagged PDF), стили
ранов (вес/курсив/декорации/цвет/кегль/семейство, sub/sup, RTL dir), отступы/интервалы/line-height/
justification, page-break-хинты → CSS break-before, таблицы (colgroup из grid, colspan/rowspan из
gridSpan+vMerge, рамки/заливка/паддинги), блочные и инлайн-картинки как data:-URI, текст шейпов. Losses:
геометрия чартов/шейпов, inline math, колонтитулы. Обвязка: `Ream.convert('html')` (ветка ДО резолва
...
**E1 — Чарты на листах xlsx + chartColorStyle (✅).** §20.5: worksheet <drawing r:id> →
драйвинг-парт; xdr:twoCellAnchor размер из траков сетки (конверсии print-model: 105 twips/char,
960/300 дефолты), one-cell/absolute — из ext; каждый чарт-фрейм → ChartBlock ПОСЛЕ таблицы листа
(anchor-ordered, ключ = путь chart-парта — глобально уникален). xlsx-reader получил theme-backed
colorResolver (паттерн docx). Оба ридера читают chartColorStyle-rel чарт-парта (colorsN.xml) →
Chart.seriesColorCycle перекрывает Office-цикл (общий withChartColorStyle в
core/drawingml/chart-parser). Корпус xlsx 349: ✅164 · ⚠️164 · ❌21 — статусы идентичны, 19
чарт-доков улучшили TextSim (chartTitle_withTitle 35.2→68.8%); Visual местами ↑ — v1-цена «после
таблицы» vs голденовский оверлей поверх сетки. 4 новых теста (517).

**P1 — Lbl для маркеров (✅).** §14.8.4.3.3: applyNumbering штампует listMarker на маркер-ран,
токены несут флаг; tagged-пагинация раскалывает ПЕРВУЮ строку пункта на две line-команды с общим
baseline: маркер-токены → Lbl (создаётся рядом с LBody), остальное → P со сдвинутым originX.
Только простая геометрия (LTR, left) — justify/center/RTL остаются как раньше; un-tagged путь
нетронут (гейт 12/12). Таб после маркера уходит в P-сегмент. veraPDF 1a/2a/3a/ua1 + комбо
зелёные.

**Инфра-серия + обтекание (✅ 5 пунктов).** (I1) Geometry-метрика корпуса: words с совпавшим
текстом матчатся по порядку, доля с |Δx|,|Δy|≤6pt — font-agnostic позиция; колонка Geom в
отчётах. (I2) GovDocs1 ПРОВЕРЕН И ОТВЕРГНУТ (legacy .doc/.xls 2009, OOXML≈0); вместо него
LibreOffice/core qa-корпуса: lo-docx-export (834) + lo-docx-import (160) + lo-xlsx (293) — корпус
476→1763 дока (×3.7); fetch --source префикс-фильтр. Первый LO-прогон: ✅476·⚠️351·❌7; триаж
одним коммитом: OOXML Strict (isOoxmlRel: schemas.openxmlformats + purl.oclc базы — strict.docx
и др. конвертятся), block-level w:sdt разворот (76 доков несут content controls!), CFB-магия →
честная ошибка для encrypted; итог ✅495·❌4(encrypted by design), ноль флипов в минус. (I3/C10)
axis-chrome: 4 push-хелпера, порядок на call-site'ах — побайтово (chart-тесты с координатами).
(I4/A9) PageAssembler: ~17 переменных + 9 замыканий пагинации перенесены ДОСЛОВНО в класс
(автоматизированный transplant, 2 предсказуемых regex-промаха); гейт 12/12 + **корпус-отчёт по
834 докам ПОБАЙТНО идентичен** (0 строк дифа). (F1) Обтекание: square/tight/through уходят из
потока + exclusion-прямоугольник; перекрытый параграф ре-обёртывается per-line ширинами (К-П
НАТИВНО принимает массив ширин — последняя переиспользуется хвостом!); один геометрический
предикат даёт и ширину, и x-сдвиг строки при эмиссии (консистентность). Текст по более широкой
стороне, зазор 6pt, пол 36pt; оценка позиций по высоте первой строки (моноразмерные параграфы
точны, смешанные — v1-аппроксимация). wrapNone без текст-эффекта, topAndBottom в потоке, таблицы
не сужаются (v1). Корпус после F1: ✅497(+2: tdf106132/tdf116985 — LO-баги ПРО обтекание) ·
ноль ухудшений TextSim>0.5pp на всех 834. 524 теста.

---

## РЕЛИЗ 1.3.0 (2026-06-12)

Срез после серии «Excel-хвост → PDF-хвост → инфра → обтекание». Версия поднята (package.json
1.3.0), CHANGELOG `[Unreleased]` закрыт как `[1.3.0]`, документация на сайте (scope/index/
examples) и README обновлены, все гейты зелёные.

**Что нового в 1.3.0 (Added):** чарты на листах xlsx (drawing-парт + chartColorStyle); обтекание
floats (square/tight/through через exclusion + per-line Кнут-Пласс); Lbl-маркеры списков в
tagged PDF; шифрование AES-256 (R6, WebCrypto, async-путь). **Changed:** OOXML Strict
(purl.oclc.org rel-базы), block-level w:sdt разворот, честная CFB-диагностика для зашифрованных
файлов.

**Инфра (dev-facing, не в пользовательском CHANGELOG):** geometry-метрика корпуса (font-agnostic
позиции, колонка Geom); корпус ×3.7 на LibreOffice/core qa-данных (1763 дока; GovDocs1 отвергнут
как pre-OOXML); axis-chrome дедуп (C10) и PageAssembler-вынос (A9) — оба корпус-подтверждены
побайтно.

**Состояние гейтов на релизе:** 524 теста (52 файла), байт-гейт 12/12, veraPDF 1a/1b/2a/2b/2u/
3a/3b/3u/ua1 + комбо A-2a+UA-1, lint 0 errors, build + test:build ок. Корпус: POI docx
✅69·⚠️41·❌17, POI xlsx ✅164·⚠️164·❌21, LO docx-export ✅497·⚠️333·❌4(encrypted by design).

**Релиз делает пользователь:** `git tag v1.3.0 && git push --tags` (npm latest), merge v1→main
PR (деплой сайта).
...
(SHA-2 семейство + AES-CBC раунды; no-pad CBC поверх обязательного PKCS7 WebCrypto — отрезание
паддинг-блока, для дешифровки — пристройка крафтового хвоста), /U /UE /O /OE /Perms со случайным
FEK, AESV3-шифрование всех строк/потоков графа (IV+CBC; /Encrypt добавляется ПОСЛЕ прохода,
трейлер /Encrypt N R). Маршрут: только async-конверсия (sync кидает); валидации в
renderStyledPdfEncrypted: pdfA+encrypt → ошибка (ISO 19005), signature+encrypt → v1-ошибка,
pdfUA → форс бита 10. Выход НЕдетерминирован осознанно (ключ/соли/IV случайные). Sync-ядра
конвертеров вынесены в prepare*StyledRender (байт-нулевой рефакторинг, гейт 12/12). Тест
round-trip: валидация пароля по /U, один FEK из /UE и /OE, контент расшифровывается до BT/Tj,
plaintext не течёт. 4 новых теста (521). Открыть руками: /tmp/enc-probe.pdf, пароль 'secret'.
...
выходные (дерево → representer), реализуемые через публичные интерфейсы.

**Каркас (принят за основу).** Деревьев не одно, а **два** — как DOM vs render
tree в браузере; универсальная «модель всего» отвергнута (путь UNO):

- **FlowDoc** — семантика без страниц/координат (параграфы, раны, стили,
  таблицы, списки). Родной для DOCX/HTML/MD. Зародыш уже есть —
  `document-model` (`BodyElement[]`): оба конвертера уже сходятся в
  `renderStyledPdf(body, …)`.
- **PageDoc** — страницы + позиционированный контент + struct-дерево. Родной
  для PDF/SVG/PNG/XPS. Зародыш есть — приватный `DrawCommand[]` внутри
  `styled-page-renderer`.
- Трансформы: **layout: FlowDoc→PageDoc** (наш движок, есть) и
  **reconstruct: PageDoc→FlowDoc** (анализ макета для PDF→Word — отдельная
  «эра», не «ещё один reader»).
- Бонусы из коробки: docx-ГЕНЕРАЦИЯ из FlowDoc (шаблонизация договоров),
  SVG/PNG-превью из PageDoc.

**Принятые решения:**

1. **SheetDoc (xlsx)** — принят вариант (б): сетка — третье дерево SheetDoc +
   трансформ print-project: SheetDoc→FlowDoc. **ТЕХДОЛГ, приоритет снижен** —
   пока xlsx-reader продолжает проецировать сетку в FlowDoc напрямую (как
   сейчас); SheetDoc выделяем, когда появится второй потребитель сетки
   (xlsx→CSV/HTML, формулы).
2. **Fidelity:** v1 обещает **cross-format** (docx→pdf и т.п.). **Round-trip
   fidelity** (docx→IR→docx ≈ исходник) — отложенная цель (≈×3 к сложности),
   НО архитектурно закладываем сразу: ноды несут опциональный
   `native`-passthrough-мешок со скоупом формата (урок pandoc RawBlock), чтобы
   дверь не захлопнуть.

3. **IR = публичный контракт** (решено):
   - интерфейсы экспортируются сразу, но **`@experimental` до стабилизации на
     3+ адаптерах** (ломать можно в minor); заморозка схемы — отдельное событие
     «IR v1 frozen», не побочный эффект релиза;
   - **единицы:** одна каноническая на всё дерево — **`pt` как branded type**
     (layout всё равно во float; 1 twip = 0.05pt — точно в double);
   - **бинарные ресурсы:** IR = **`{ tree, resources }`**; ResourceStore
     content-addressed (id = хэш байтов), ноды ссылаются по id; дерево —
     чистый JSON (диффабельно), дедупликация картинок бесплатно;
   - **стили: resolved.** FlowDoc несёт разрезолвленные свойства (writer не
     обязан реализовывать каскад) + опциональная секция `styles` для writers,
     генерящих именованные стили. Минус (docx→docx теряет именованность)
     принят — round-trip и так отложен, passthrough держит дверь.
4. **Материализация, не streaming** (решено): полное дерево в памяти +
   конфигурируемые self-limit'ы в reader'ах (контракт адаптера включает
   «обязан ограничивать вход», линия #114). SAX-стриминг — если появится кейс
   «xlsx на миллионы строк», отдельным путём рядом, не заменой. **Адаптеры
   sync-first** (парсинг чистый, байты в памяти); вся асинхронщина (фетч
   шрифтов/ресурсов) — на фасаде-оркестраторе.
5. **Capabilities + потери** (решено): статическая декларация
   `supports: Set<Feature>` (иерархический словарь: `tables.nested`, `math`,
   `rtl`, `images.jp2`…; capability-matrix в доки генерится из кода) +
   динамический **`LossReport`** на конвертации со severity
   `dropped | degraded | substituted` (font-substitution = `substituted`).
   **`strict`-режим — параметр пользователя** (по умолчанию false; юр./архивные
   проекты ставят `strict: true` → ошибка на первой потере вместо warning).

6. **Шрифтовой пайплайн** (решено): резолв шрифтов — **FontProvider-цепочка**
   (адаптерная идеология и здесь). Запрос `(family, style, codepoints)` идёт по
   ступеням, первый отвечающий выигрывает; всё ниже «байтов» пишет в
   `LossReport` (`substituted`/`degraded`), `strict: true` превращает в ошибку:
   - **v1:** интерфейс + существующие провайдеры под него: embedded-in-docx →
     caller bytes → CDN-замены;
   - **v1 (новое):** провайдер **Local Font Access API** (Chromium,
     `queryLocalFonts()`, с permission) — настоящие байты системных шрифтов
     («PDF выглядит как в Word без загрузки шрифтов»). Обязателен фильтр
     **OS/2 `fsType`** (embedding-биты): restricted-шрифты не встраиваем →
     substituted/strict-ошибка;
   - **экспериментально (за флагом):** canvas-metrics — layout по
     `measureText` настоящего шрифта без его байтов (верная геометрия
     страниц), embed — замена. Без kerning/shaping наружу: ок для
     латиницы/кириллицы, слабо для арабицы. Точность мерить на корпусе;
   - **бэклог:** растровый fallback на отдельные отсутствующие глифы
     (эмодзи/редкий CJK; глиф → inline-image + ActualText). Tofu-killer,
     применяется хирургически;
   - **отвергнуто:** автотрейс пикселей в контуры / синтез TTF («пересборка»):
     качество мусорное (хинтинг/АА шумят), объём — milestone, лицензионно — то
     же встраивание чужого шрифта, только хуже.

Проверка дизайна: третий дешёвый адаптер (FlowDoc→HTML или PageDoc→SVG) в
плане сразу — он вскрывает ошибки интерфейсов. Миграция инкрементальная, под
byte-identical корпус-гейтами.
