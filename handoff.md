...
**Заметки к дискуссии (открытые вопросы):**

6. **Шрифтовой пайплайн** — оформить резолв шрифтов как **FontProvider-цепочку**
   (адаптерная идеология и здесь): embedded-in-docx → caller bytes → Local Font
   Access API (Chromium) → CDN-замены → canvas-metrics (layout по measureText
   без байтов) → растровый fallback на отдельные глифы. Открыто: скоуп v1,
   лицензионный аспект (OS/2 `fsType` embedding-биты), точность canvas-метрик.
