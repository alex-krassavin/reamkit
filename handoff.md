...
**Заметки к дискуссии (открытые вопросы, решаются до кода):**

3. **IR = публичный контракт.** «Ты имплементируешь интерфейс» ⇒ схема деревьев
   под semver: ядро минимальное и консервативное; единицы измерения —
   нейтральные канонические (не twips/не pt-вперемешку); JSON-сериализуемость
   (хранение, дифф, передача).
4. **Streaming vs материализация.** Полное дерево в памяти (DOM-путь) +
   self-limit'ы (как сейчас, после xlsx-OOM #114) vs SAX-стриминг. Склоняемся к
   материализации — зафиксировать как осознанный трейд-офф.
5. **Capabilities + потери.** Адаптер декларирует, что умеет
   (`supports: …`); конвертация возвращает отчёт о потерях
   (`warnings: [math dropped]`) вместо тихого съедания.
