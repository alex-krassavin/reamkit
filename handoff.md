...
конфиги (корень)
  ...
  tsconfig.json              module ESNext + moduleResolution Bundler, noEmit,
                             paths @/*→src/*, strict
  vite.config.ts             tanstackViteConfig (3 entry) + dts-хук (расширения,
                             directory→/index.js) + sourcemap off
  vitest.config.ts           resolve.alias @→src; include tests/**/*.test.ts
  eslint.config.js           typescript-eslint recommended (стиль за prettier)
  .prettierrc.json/.ignore   singleQuote/semi/trailingComma all/printWidth 100
```

**367/367 тестов зелёные** (включая live-прогон veraPDF всех 8 PDF/A-профилей, если установлен). Strict TypeScript (exactOptionalPropertyTypes + noUncheckedIndexedAccess) — чисто.
...
- **Тулинг:** Prettier (`.prettierrc.json` — singleQuote/semi/trailingComma all/
  printWidth 100; `npm run format`/`format:check`); ESLint flat-config
  (`eslint.config.js` на typescript-eslint recommended, стиль отдан prettier,
  `no-non-null-assertion` off; `npm run lint`/`lint:fix`).
...
- **Открытые решения перед публикацией:** (1) `private: true` снять для publish;
  (2) `license` не задан — выбрать; (3) `version` 0.0.0 — забампить;
  (4) `repository`/`author`.
